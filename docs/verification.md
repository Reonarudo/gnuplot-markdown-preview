...
- Final visual verification passed in the dedicated **Gnuplot Marketplace Test** profile after installation by Marketplace extension ID. Built-in Markdown Preview displayed the function plot, multiple functions, inline dataset, localized error followed by continued content, and 3D surface correctly in the dark theme. Swift and JavaScript fences retained syntax highlighting. This completes the Marketplace-installed visual check separately from the earlier development-host and local-VSIX checks.
