...
WASM uses 64 MiB fixed linear memory in an isolated Node worker with an empty process environment. The worker reads only the trusted adapter and fixed WASM file from the host. Gnuplot sees an in-memory filesystem restricted to script.gp, plot.svg, and explicitly supplied read-only data snapshots; it cannot read workspace or host files directly. In 0.2.0, the extension host reads literal CSV/DAT/TXT dependencies through vscode.workspace.fs only in trusted local workspaces. It checks containing-workspace boundaries, rejects symlink components below the workspace root, and checks path containment and file metadata before and after reading. This is not race-proof isolation against a hostile process replacing directories during reads; workspace trust remains required. Remote URI providers are rejected for external data. Shell, pipes, and backticks have no functioning host bridge. Output remains untrusted and is parsed as XML, with passive SVG elements/attributes allowed and scripts, foreignObject, external references, animation and event handlers removed. DOCTYPE/entity declarations and malformed output are rejected. Every plot gets unique IDs to avoid collisions between plots and cached occurrences.
...
File snapshots are limited to 16 files, 1 MiB each, and 4 MiB per plot; retained snapshots are limited to 32 MiB and 128 plot entries, with eight loads in flight. Dependency changes invalidate ready and error entries; stale reads cannot repopulate invalidated entries. Render cache keys include document identity and data content hashes. Snapshot files are deleted after every render, including failures, and write-mode opens are denied. Default data titles are restored in SVG text nodes through XML serialization, never inserted as raw markup.
